...
Dashboard.register('heatmap', (() => {
  ...
  function render(s) {
    ...
    chart.setOption({
      ...
      series: [
        ...
        {type: 'effectScatter', symbolSize: 16, data: [[curX, curY, 0]], itemStyle: {color: '#fff'}, rippleEffect: {brushType: 'stroke', scale: 3}, label: {show: true, position: 'right', color: '#fff', fontSize: 11, fontWeight: 'bold', backgroundColor: 'rgba(233,69,96,0.85)', padding: [3, 6], borderRadius: 4, formatter: '当前: ' + Math.round(curMonthly).toLocaleString('zh-CN') + ' 元/月'}}
      ]
    }, true);
  }
  ...
  return {
    ...
    update(s) {
      lastS = s;
      // 防抖：连续拖滑块时只在停顿后重算 221 格
      clearTimeout(timer);
      timer = setTimeout(() => render(lastS), 120);
    },
