...
Dashboard.register('asset', (() => {
  ...
  function renderAssetControls(root) {
    ...
    assetNames.forEach(name => {
      ...
      row.innerHTML = `<div class='flex justify-between items-center mb-2'><span class='text-sm text-white font-medium'>${name}</span><span class='text-sm text-[#e94560] font-mono asset-pct' data-name='${name}'>${state.assetWeights[name]}%</span></div><input type='range' min='0' max='100' step='1' value='${state.assetWeights[name]}' class='asset-slider w-full mb-2' data-name='${name}'><div class='asset-meta flex justify-between text-xs text-gray-400' data-name='${name}'></div>`;
  ...
  return {
    ...
    update(s, root) {
      ...
      deposit.setOption({
        ...
        series: [
          {name: '账面每年需存', type: 'line', data: nominalSeries, smooth: true, lineStyle: {color: '#e94560'}, itemStyle: {color: '#e94560'}, symbol: 'none', areaStyle: {color: 'rgba(233,69,96,0.15)'}},
